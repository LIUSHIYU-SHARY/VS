#mkdir -p result/20_phase_512_generate_images
#mkdir -p /home/yuqi/virtual_staining/output/pix2pix/mito-20x-phase/512_checkpoint
#python further_train_512_20x_phase.py --dataset ../../tmp_data/Nucleus/20x/20x-phase-512 --cuda

# /home/yuqi/virtual_staining/tmp_data/Nuclear-classification/20x/20x-phase-512

#python further_train_512_20x_phase.py --dataset ../../tmp_data/Nuclear-classification/20x/20x-phase-512 --cuda
#cuda:0

#2月19号训练20x phase 512模型
#0310加上了consistency loss
#0310去掉了consistency loss，epoch改为400
#0320改变了预训练模型
#0322去掉了预训练模型
#0328筛选了照片，换成了New_Nucleus
#0407筛掉了不好的照片，剩下157组然后裁剪成512*512
...
parser.add_argument('--save_dir', type=str, default='../../output/pix2pix/Nucleus/0407_512_20x_phase_generate_images', help='save generate image')
...
if os.path.exists('../../output/pix2pix/Nucleus/0319_minigut_checkpoint/netG_model_epoch_200.pth'):
    checkpoint_g = torch.load('../../output/pix2pix/Nucleus/0319_minigut_checkpoint/netG_model_epoch_200.pth', map_location=device, weights_only=True)
    net_g.load_state_dict(checkpoint_g)
    net_g.load_state_dict(torch.load('../../output/pix2pix/Nucleus/0319_minigut_checkpoint/netG_model_epoch_200.pth', map_location=device, weights_only=True))
...
if os.path.exists('../../output/pix2pix/Nucleus/0319_minigut_checkpoint/netD_model_epoch_200.pth'):
    # 先加载模型权重到内存中
    checkpoint_d = torch.load('../../output/pix2pix/Nucleus/0319_minigut_checkpoint/netD_model_epoch_200.pth', map_location=device, weights_only=True)
    net_d.load_state_dict(checkpoint_d)
    net_d.load_state_dict(torch.load('../../output/pix2pix/Nucleus/0319_minigut_checkpoint/netD_model_epoch_200.pth', map_location=device, weights_only=True))
...
writer = SummaryWriter(log_dir='logs/0407-pix2pix-512-20x-phase-Nucleus')
...
#change 
    # checkpoint 
    if epoch % 5 == 0:
        if not os.path.exists('../../output/pix2pix/Nucleus/0407_512_20x_phase_checkpoint'):
            os.makedirs('../../output/pix2pix/Nucleus/0407_512_20x_phase_checkpoint')

        torch.save(net_g.state_dict(), '../../output/pix2pix/Nucleus/0407_512_20x_phase_checkpoint/netG_model_epoch_{}.pth'.format(epoch))
        torch.save(net_d.state_dict(), '../../output/pix2pix/Nucleus/0407_512_20x_phase_checkpoint/netD_model_epoch_{}.pth'.format(epoch))
