...
print("测试集路径:", opt.dataset)
print("数据方向:", opt.direction)
if len(test_set) == 0:
    print("警告: 测试集为空!")
...
save_dir='results/512_bright_40x_mito_without_structural'
#writer = SummaryWriter(log_dir='logs/pix2pix-512-20x-phase-nuclei')
...
#for epoch in range(num_batch):
    ...
    #checkpoint
    if epoch % 5 == 0:
        if not os.path.exists('../../output/pix2pix/gray/gray_40x_bright_mito_without_structural'):
            os.mkdir('../../output/pix2pix/gray/gray_40x_bright_mito_without_structural')

        torch.save(net_g.state_dict(), '../../output/pix2pix/gray/gray_40x_bright_mito_without_structural/netG_model_epoch_{}.pth'.format(epoch))
        torch.save(net_d.state_dict(), '../../output/pix2pix/gray/gray_40x_bright_mito_without_structural/netD_model_epoch_{}.pth'.format(epoch))
