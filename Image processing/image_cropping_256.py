...
def crop_paired_images(imgA_path, imgB_path, output_dirA, output_dirB, crop_size=256, pad=24):
    """
    切割配对图像，并分别保存到两个文件夹中
...
def process_folders(folderA, folderB, output_dirA, output_dirB, crop_size=256, pad=24):
    """
...
#     original_path = "20x/20x-phase" 
#     # 裁剪后数据集路径 
#     magnification_cropping_path = "20x/20x-phase-256" 
...
if __name__ == "__main__":
    # 定义原始数据集和裁剪后数据集的映射
    dataset_mapping = {
        "20x/20x-phase": "20x/20x-phase-256",
        "20x/20x-bright": "20x/20x-bright-256",
        "40x/40x-bright": "40x/40x-bright-256"
    }
