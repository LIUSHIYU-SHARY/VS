...
plt.rcParams.update({
    'font.family': 'sans-serif',
    'font.size': 10,             # 原为7
    'axes.titlesize': 12,        # 原为8
    'axes.labelsize': 10,        # 原为7
    'xtick.labelsize': 9,        # 原为6
    'ytick.labelsize': 9,        # 原为6
    'legend.fontsize': 9,        # 原为7
    'lines.linewidth': 1.0,      # 原为0.8
    'axes.linewidth': 0.8,       # 原为0.6
    'grid.linewidth': 0.6        # 原为0.4
})
...
def save_radar(data_subset, title, filename):
    ...
    ax.set_title(title, size=12, pad=15)
