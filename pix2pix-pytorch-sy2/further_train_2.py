...
parser.add_argument('--save_dir', type=str, default='result/generate_512_images', help='save generate image')
...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    # checkpoint
    if epoch % 5 == 0:
        if not os.path.exists('../../output/pix2pix/mito-40x/512_checkpoint'):
            os.mkdir('../../output/pix2pix/mito-40x/512_checkpoint')

        torch.save(net_g.state_dict(), '../../output/pix2pix/mito-40x/512_checkpoint/netG_model_epoch_{}.pth'.format(epoch))
        torch.save(net_d.state_dict(), '../../output/pix2pix/mito-40x/512_checkpoint/netD_model_epoch_{}.pth'.format(epoch))
