...
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2,3' # 使用0,1,2,3号GPU
...
training_data_loader = DataLoader(
    ...
    batch_size=opt.batch_size * torch.cuda.device_count(), # 增加batch size
    shuffle=True,
...
#
## 计算速度：样本/秒
#samples_per_second = (opt.batch_size * num_batch) / total_time
#print(f"Training Speed: {samples_per_second:.2f} samples/second")
    ...
    #checkpoint
#    if epoch % 2 == 0:
#        if not os.path.exists('output/my_checkpoint/pix2pix'):
#            os.mkdir('output/my_checkpoint/pix2pix')
#
#        torch.save(net_g.state_dict(), 'output/my_checkpoint/pix2pix/netG_model_epoch_{}.pth'.format(epoch))
#        torch.save(net_d.state_dict(), 'output/my_checkpoint/pix2pix/netD_model_epoch_{}.pth'.format(epoch))
#        
...
#        print("Checkpoint saved to {}".format("checkpoint" + opt.dataset))
    if epoch % 2 == 0:
        ...
        torch.save(net_g.module.state_dict() if isinstance(net_g, nn.DataParallel) 
              else net_g.state_dict(), 
              'output/my_checkpoint/pix2pix/netG_model_epoch_{}.pth'.format(epoch))
        torch.save(net_d.module.state_dict() if isinstance(net_d, nn.DataParallel) 
              else net_d.state_dict(),
              'output/my_checkpoint/pix2pix/netD_model_epoch_{}.pth'.format(epoch))
